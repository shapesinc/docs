...
The big idea: **AI is better with other people in the room.** Everything below builds on that.

## 1. The group chat is the room
...
Each chat has its own name, look, and rules, so every room can feel like its own place. More in [AI Group Chats](/ai-group-chats).
...
A great Shape knows when to jump in and when to stay quiet. The main control is **Free Will**, which decides when a Shape speaks on its own (when it's mentioned, when a keyword comes up, when the room goes quiet, and more). Tune it to the room and you get a Shape that reads the vibe. Learn the craft in [Designing Social Intelligence](/designing-social-intelligence).
