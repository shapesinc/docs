...
| **Short backstory** | One or two sentences on who the Shape is. The most important thing you'll write — give it a clear point of view. |
...
- **Knowledge relevance** — how closely an entry must match to be used. Lower recalls more but looser; higher recalls fewer but tighter.

## Training

Input/output examples that *guide* how the Shape replies — it learns the pattern, it doesn't repeat them verbatim. The more representative examples you add, the more consistent its voice.
...
These shape the raw output of the model. The defaults are good — **don't change these unless you know what they do.**

<AccordionGroup>
  <Accordion title="Sampling parameters" icon="sliders">
    - **Temperature** — randomness/creativity (default `0.95`). Higher is wilder, lower is more focused.
    - **Top P** — nucleus sampling cutoff.
    - **Top K** — limits choices to the K most likely tokens.
    - **Min P** / **Top A** — additional probability filters.
    - **Repetition penalty** — discourages repeating the same text.
    - **Frequency penalty** / **presence penalty** — push the model toward fresh wording and new topics.
...
| **LTM engine + instructions** | The model and the prompt used to summarize conversations into memories. Worth tuning — this controls *what* gets remembered. |
...
<AccordionGroup>
  <Accordion title="Advanced voice controls" icon="waveform">
    - **Speed** — how fast it speaks.
    - **Variation** — how much the delivery varies between messages.
    - **Exploration** — how freely the voice model interprets the text.
    - **Voice instructions** — guidance for how the Shape should sound when speaking, and how voice messages are represented in history.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Copy-ready example Shapes" icon="grid-2" href="/showcase">
