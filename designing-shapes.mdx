...
<CardGroup cols={2}>
  <Card title="A point of view" icon="eye">
    It wants something, talks a certain way, and reacts like a specific person — not a helpful blank.
  </Card>
...
```mermaid
flowchart TD
    A[Identity<br/>name · short backstory] --> B[Personality<br/>traits · tone · likes · goals]
    B --> C[Response style / Preset<br/>how it actually writes]
    C --> D[Knowledge & Training<br/>facts it must know · examples of its voice]
    D --> E[Memory<br/>short-term + long-term]
...
The first one already implies a voice, a mood, and a setting. The second one implies nothing — the model will fall back to its default "assistant" persona, and your Shape will feel like everyone else's.
...
You can start from a named response style in the dashboard (options range from `Human (No Roleplay)` to `Long Roleplay`, `Balanced`, and `Custom`) and then edit the text. Picking a named style can also auto-suggest a fitting [model](/choosing-a-model). For a deeper toolkit of prompt patterns — declarative, imperative, prohibitive, obligation — read [Prompt 101](/prompt101), and grab ready-made examples from [Presets](/presets).
...
- **Web, code, files, and more** — with [Shape Skills](/shapeskills) it can search the web, run code, read PDFs, roll dice, and connect custom tools.
...
A Shape that's perfect in a one-on-one DM can be unbearable in a group chat if it answers *every* message. The discipline of knowing when to speak is big enough to have [its own guide](/designing-social-intelligence) — read it before you put a Shape into a busy room. The short version:
